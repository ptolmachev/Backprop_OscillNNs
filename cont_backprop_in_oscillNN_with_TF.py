...
for i in range(num_nrns): # number of f rhs
    for j in range(num_nrns*num_nrns): #number of parameter wrt to differentiate j -> (k,n)
        for m in range(num_nrns):  # sweeping index
            RHS_Z_inh[i, j] +=  F_diff_m[i, m] * Y_inh[m, j]
...
V_half = -30
W_inh = np.array([[0.1, 0.1, 0.1, 0.1]])
dt = 0.1
vals_V = [[V[0,0]],[V[0,1]]]
vals_M = [[M[0,0]],[M[0,1]]]
...
for i in range(int(stoptime/dt)):
    ...
    for j in range(len(F)):
        V[0,j] = H[0,j] + dt*F[j](H, M, W_inh, V_half) # H instead of V
        vals_V[j].append(V[0,j])

    for j in range(Z_inh.shape[0]):
        for k in range(Z_inh.shape[1]):
            Z_inh[j,k] = Z_inh[j,k] + dt*RHS_Z_inh[j,k](H, M, W_inh, Z_inh, Y_inh, V_half)
            vals_Z_inh[j,k].append(Z_inh[j,k])

    for j in range(Y_inh.shape[0]):
        for k in range(Y_inh.shape[1]):
            Y_inh[j,k] = Y_inh[j,k] + dt*RHS_Y_inh[j,k](H, M, W_inh, Z_inh, Y_inh, V_half)
            vals_Y_inh[j,k].append(Y_inh[j,k])

    etha = 2*1e-7

    if i > int(5000/dt): #for the network to settle into a limit cycle
...
styles = ["k","b","r","g"]
for i in range(4):
    plt.plot(t,vals_W_inh[0, i], styles[i], linewidth = 3, alpha = 0.7)
plt.grid(True)
plt.show()
print(W_inh)
